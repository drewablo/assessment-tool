...
def _build_schools_payload(zip_codes: list[str], current_by_zip: dict[str, dict[str, float]], history_by_zip: dict[str, dict[int, dict[str, float]]], result: AnalysisResponse, data_year: int) -> DashboardModuleData:
    ...
    return DashboardModuleData(
        ...
        description="Wave 1 emphasizes affordability first so income depth, tuition fit, and family growth are easy to review across the catchment.",
        primary_label="Focus",
...
def _build_elder_payload(zip_codes: list[str], current_by_zip: dict[str, dict[str, float]], history_by_zip: dict[str, dict[int, dict[str, float]]], result: AnalysisResponse, data_year: int) -> DashboardModuleData:
    ...
    return DashboardModuleData(
        ...
        description="Wave 1 starts with partnership viability so senior demand can be reviewed alongside likely care operators and facility quality.",
        primary_label="Focus",
        primary_value="Partnership viability",
        secondary_label="ZIPs",
        secondary_value=str(len(zip_codes)),
        sidebar_items=[
            ...
            DashboardSidebarItem(key="partnership_viability", title="Partnership Viability", description="Operator, ownership, and quality signals for partnership screening."),
        ],
        tabs=[
            DashboardTabItem(key="summary", label="Summary"),
            DashboardTabItem(key="distribution", label="Distribution"),
...
def _build_housing_payload(zip_codes: list[str], current_by_zip: dict[str, dict[str, float]], history_by_zip: dict[str, dict[int, dict[str, float]]], result: AnalysisResponse, data_year: int) -> DashboardModuleData:
    ...
    return DashboardModuleData(
        ...
        description="Wave 1 folds demographic trends into Community Profile so renter demand, population change, and affordability context are reviewed together.",
        primary_label="Focus",
        primary_value="Community profile",
        secondary_label="ZIPs",
        secondary_value=str(len(zip_codes)),
        sidebar_items=[
            DashboardSidebarItem(key="community_profile", title="Community Profile", description="Population, tenure, poverty, and housing-need trend context.", badge="Core"),
            DashboardSidebarItem(key="need_assessment", title="Need Assessment", description="Burdened and HUD-eligible households by ZIP."),
            DashboardSidebarItem(key="existing_resources", title="Existing Resources", description="Subsidized inventory, supply-gap context, and QCT/DDA summary framing."),
        ],
        tabs=[
            DashboardTabItem(key="summary", label="Summary"),
            DashboardTabItem(key="distribution", label="Distribution"),
