  ...
  return (
    <div className="space-y-1">
      <div className="flex justify-between items-baseline">
        ...
        <span className="text-sm font-bold text-gray-900">{percentile.toFixed(0)}th</span>
      </div>
      ...
      {sampleSize !== null && (
        <p className="text-[10px] text-gray-400">vs. {sampleSize.toLocaleString()} markets</p>
      )}
    </div>
  );
}
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="font-semibold text-gray-900">Benchmark Percentile Rankings</h3>
      </div>
      ...
      {benchmarks.comparable_markets.length > 0 && (
        <div className="border-t border-gray-100 pt-4 mt-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h4 className="text-xs font-semibold text-gray-700">Similar Market Profiles</h4>
          </div>
            ...
            <table className="w-full text-xs">
              ...
              <tbody>
                {benchmarks.comparable_markets.slice(0, 5).map((m) => (
                  <tr key={m.geoid} className="border-b border-gray-50 hover:bg-gray-50">
                    <td className="py-1.5 pr-3 text-gray-600 font-mono">{m.geoid}</td>
                    <td className="py-1.5 px-2 text-right font-bold text-gray-800">{m.overall_score}</td>
                    <td className="py-1.5 px-2 text-right text-gray-600">{m.market_size_score}</td>
                    <td className="py-1.5 px-2 text-right text-gray-600">{m.income_score}</td>
                    <td className="py-1.5 px-2 text-right text-gray-600">{m.competition_score}</td>
                    <td className="py-1.5 pl-2 text-right text-gray-600">{m.family_density_score}</td>
                  </tr>
                ))}
              </tbody>
            </table>
